...
if(empty($_POST))
	...
	if($error)
		...
		redirect('../back/updateMescreations.php?id='.$_POST['id']);

	else{
		$son = "";

		//on recupere les infos de l'ancienne cover
		$creations = $db->prepare('SELECT * FROM mescreations WHERE id = :i');
		$creations->bindParam(':i', $_POST['id'], PDO::PARAM_INT);
		$creations->execute();
		$data = $creations->fetch(PDO::FETCH_ASSOC);
		...
		$update = $db->prepare('UPDATE mescreations SET updated = NOW(), titre = :t, fichier = :f WHERE id = :i');
